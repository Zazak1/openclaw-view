...
export function renderApp(state: AppViewState) {
  ...
  const activeSurface = sectionForTab(state.tab);
  const visibleGroups = TAB_GROUPS.filter((group) => group.surface === activeSurface);
  ...
  return html`
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      ...
      <aside class="nav ${state.settings.navCollapsed ? "nav--collapsed" : ""}">
        <div class="nav-surface-switch">
          ${NAV_SURFACES.map((surface) => {
            const isActive = surface.id === activeSurface;
            return html`
              <button
                class="nav-surface-item ${isActive ? "active" : ""}"
                @click=${() => {
                  if (isActive) {
                    return;
                  }
                  state.setTab(defaultTabForSurface(surface.id));
                }}
                aria-pressed=${isActive}
                title=${surface.subtitle}
              >
                <span class="nav-surface-item__label">${surface.label}</span>
                <span class="nav-surface-item__sub">${surface.subtitle}</span>
              </button>
            `;
          })}
        </div>
        ...
        ${
          state.tab === "ops"
            ? renderOps({
                ...
                onRefreshAll: () => state.loadOverview(),
                onProbeChannels: () => loadChannels(state, true),
